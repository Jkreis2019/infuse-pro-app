        ...
        {familyMembers.length === 0 ? (
          ...
          return (
            <View key={fm.id} style={{ backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 14, marginBottom: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <TouchableOpacity onPress={async () => { try { const r = await fetch(`${API_URL}/family-members/${fm.id}`, { method: 'DELETE', headers }); const d = await r.json(); console.log('Delete result:', d); fetchLinkedCompanies() } catch(e) { console.error('Delete error:', e) } }}>
