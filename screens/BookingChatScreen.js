...
export default function BookingChatScreen({ route, navigation }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      console.error('Send booking chat error:', err)
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={[styles.header, { backgroundColor: secondaryColor }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700' }}>{patientName || 'Patient'}</Text>
      ...
      {showMenu && (
        <TouchableOpacity style={{ position: 'absolute', top: 90, right: 16, backgroundColor: '#1a2a5e', borderRadius: 10, padding: 4, zIndex: 100, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', minWidth: 180 }} onPress={() => {}}>
            ...
            {reportDone ? (
              ...
              <>
                <View style={{ padding: 20, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.08)' }}>
                  ...
                  <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, marginTop: 4 }}>This will send a full transcript of this chat to our safety team for review.</Text>
                </View>
